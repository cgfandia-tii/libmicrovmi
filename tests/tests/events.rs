...
fn intercept_cr3_one(mut drv: Box<dyn Introspectable>, cfg: CommonConfig) {
    ...
    let event = drv.listen(cfg.event_timeout).unwrap().unwrap();
    assert!(matches!(
        event.kind,
        EventType::Cr {
            cr_type: CrType::Cr3,
...
fn intercept_cr3_multiple(mut drv: Box<dyn Introspectable>, cfg: CommonConfig) {
    for vcpu in 0..drv.get_vcpu_count().unwrap() - 1 {
        drv.toggle_intercept(vcpu, InterceptType::Cr(CrType::Cr3), true)
            .expect("Failed to toggle CR3 intercept");
    }
